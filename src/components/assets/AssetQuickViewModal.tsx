...
// ✅ Try to reuse your REAL asset detail/card component (so the modal shows the full card)
import * as AssetDetailModule from "@/components/assets/AssetDetail";
const AssetDetailAny =
  (AssetDetailModule as any).default ??
  (AssetDetailModule as any).AssetDetail ??
  (AssetDetailModule as any).AssetDetailPanel ??
  null;
...
export default function AssetQuickViewModal({ symbol, isOpen, onClose }: AssetQuickViewModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent
        // Keep this modal clearly above the EventDetails modal
      ...
      >
        {/* ✅ If AssetDetail exists, render the FULL card */}
        {AssetDetailAny ? (
          <div className="p-0">
            <AssetDetailAny
              symbol={symbol}
              // Safe “optional” props — ignored if the component doesn’t use them
              inModal
              modal
              isModal
              onClose={onClose}
          ...
          // Fallback: your current “quick summary” view
          <>
            {!asset ? (
